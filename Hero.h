...
public:
    ...
    Hero(std::string name, double hp, Damage dmg, double a, double exp, double lvl, double experience_per_level, double health_point_bonus_per_level, 
        double damage_bonus_per_level, double cooldown_multiplier_per_level, double dfs, double dfsbl, double mbpl, double lr, double lrpl) 
                : unit(name, hp, dmg, a, dfs), exp(exp), lvl(lvl), maxhp(hp), experience_per_level(experience_per_level), 
                    ...
                    defense_bonus_per_level(dfsbl), magical_damage_bonus_per_level(mbpl),  light_radius(lr), light_radius_bonus_per_level(lrpl){}
    Hero(Hero* l): unit(l->getName(),l->getHealthPoints(),Damage(l->getPDamage(),l->getMDamage()),
                    ...
                    light_radius(l->getLightRadius()), light_radius_bonus_per_level(l->getLightBonus()) {
                        delete l;
                    }
